...
def simulate_batch(T, n_steps, n_paths, xp, drift=0.0, variance_function=None):
    dt = T / n_steps
    # shape (n_paths, n_steps)
    inc = xp.random.normal(drift, 1.0, size=(n_paths, n_steps)) * xp.sqrt(dt)
    if variance_function is not None:
        # shape (n_steps,)
        scale = xp.asarray(variance_function(xp.linspace(dt, T, n_steps)))
        inc *= scale[None, :]
...
def simulate_single_path(T, n_steps, initial_value, dt,
    ...
    inc = xp.random.normal(drift, 1.0, size=n_steps)
    if variance_function:
        var_scale = variance_function(times[1:])
...
def brownian_motion_paths_cpu_stream(
    ...
    for start in range(0, n_paths, batch):
        ...
        paths = simulate_batch(
            ...
            size,              # batch size
            np,                # numpy backend
            drift=0.0,         # mean drift parameter
            variance_function=None  # variance modifier function
        )

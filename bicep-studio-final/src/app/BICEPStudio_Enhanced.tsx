...
import { 
  ...
  Image as ImageIcon, Pause, Brain, Loader2, CheckCircle,
  Server, FileDown
} from "lucide-react";
...
export default function BICEPStudio() {
  ...
  const [useRemotePolicy, setUseRemotePolicy] = useState(false);
  ...
  async function trainPolicy() {
    if (isTraining || serverStatus !== 'online') return;
    
    setIsTraining(true);
    setTrainingProgress('Generating demonstrations...');
    ...
    try {
      const trainReq = {
        ...
        num_demos: 100,
        bc_epochs: 200,  // Increased from 50
        rl_episodes: 100,
        beta_schedule: [0.5, 0.3, 0.1, 0.0]
      };

      setTrainingProgress('Training neural network...');
      ...
      setUseRemotePolicy(true);
      setTrainingProgress(`Training complete! Policy: ${result.policy_id}`);
      ...
      setTrainingProgress('Training failed - check server');
      setTimeout(() => {
        setTrainingProgress('');
        setIsTraining(false);
      }, 3000);
  ...
  async function evaluatePolicy() {
    if (!ens && !currentPolicyId) {
      alert('Load a policy or train one first');
      return;
    }

    if (currentPolicyId && serverStatus === 'online') {
      ...
      try {
        ...
        const result = await response.json();
        const stats = `Server eval: ${(result.overall_success_rate * 100).toFixed(1)}% success | ${result.mean_steps?.toFixed(1) || 'N/A'} avg steps`;
        setPolicyStats(stats);

      } catch (error) {
        console.error('Server evaluation failed:', error);
        setPolicyStats('Server evaluation failed');
      }
    } else if (ens) {
      // Use local evaluation (existing code)
      const episodes = 20;
      let successes = 0;
      let totalSteps = 0;
      ...
      const stats = `Local eval: ${successRate}% success | ${avgSteps} avg steps`;
      setPolicyStats(stats);
    }
  }

  async function exportServerDemos() {
    ...
    try {
      const response = await fetch(`${SERVER_URL}/bicep/demos`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          grid,
      ...
      const csvHeader = 'demo_id,cost,risk,clearance,path_length,path_points\n';
      const csvRows = result.demos.map((demo: any, i: number) => 
        `${i},${demo.cost.toFixed(3)},${demo.risk.toFixed(3)},${demo.clearance.toFixed(2)},${demo.path.length},"${demo.path.map((p: number[]) => `(${p[0]},${p[1]})`).join(';')}"`
      ).join('\n');
      ...
      a.href = url;
      a.download = `bicep-server-demos-${Date.now()}.csv`;
      a.click();
      URL.revokeObjectURL(url);

    } catch (error) {
      console.error('Server demo export failed:', error);
      alert('Server demo export failed');
    }
  }
      ...
      <header className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-zinc-400 mt-2">
            Parallel path sampling + ensemble policy training with deterministic replays
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-6">
        ...
        <div className="relative">
          ...
          >
            ...
            {/* Policy Status */}
            {(currentPolicyId || ens) && (
              <div className="absolute bottom-3 right-3 text-xs bg-blue-500/10 border border-blue-500/40 px-3 py-1 rounded">
                Policy: {currentPolicyId ? 
                  <span className="font-bold text-blue-400">{currentPolicyId}</span> : 
                  <span className="font-bold text-purple-400">local</span>
                }
              </div>
            )}
          </div>
        ...
        <aside className="space-y-4">
          ...
          <Section title="ENN Policy">
            <div className="grid grid-cols-2 gap-2">
              ...
              <Button variant="secondary" onClick={async () => {
                ...
                if (ensemble) {
                  ...
                  setUseRemotePolicy(false);
                  alert(`Local policy loaded: ${ensemble.heads.length} heads`);
                }
              }}>
            ...
            <div className="grid grid-cols-2 gap-2 mt-2">
              ...
              <div className="flex items-center">
                <input 
                  type="checkbox" 
                  id="use-remote" 
                  checked={useRemotePolicy}
                  onChange={(e) => setUseRemotePolicy(e.target.checked)}
                  disabled={!currentPolicyId}
                  className="mr-2"
                />
                <label htmlFor="use-remote" className="text-xs text-zinc-300">
                  Use Remote
                </label>
              </div>
            </div>
            ...
            {trainingProgress && (
              <div className="mt-2 p-2 bg-blue-800 border border-blue-700 rounded text-xs flex items-center">
            ...
            {currentPolicyId && (
              <div className="mt-2 text-xs text-blue-400">
                Remote policy: {currentPolicyId}
              </div>
            )}

            {ens && (
              <div className="mt-2 text-xs text-purple-400">
                Local: {ens.heads.length} heads, {ens.hidden} hidden
              </div>
            )}
          </Section>
          ...
          <Section title="Share & Export">
            ...
            <div className="grid grid-cols-2 gap-2 mt-2">
              <Button variant="secondary" onClick={exportServerDemos} disabled={serverStatus !== 'online'}>
                <Server className="w-4 h-4 mr-1" />
                Server CSV
              </Button>
              <Button variant="secondary" onClick={() => {
                // Existing client CSV export
                if (winners.length === 0) {
                  alert('No paths to export! Run BICEP first.');
                  return;
                }
                
                const csvHeader = 'id,cost,risk,clearance,length,path\n';
                const csvRows = winners.map((w, i) => 
                  `${i},${w.cost.toFixed(3)},${w.risk.toFixed(3)},${w.clearance.toFixed(2)},${w.path.length},"${w.path.map(p => `(${p[0]},${p[1]})`).join(';')}"`
                ).join('\n');
                ...
                const a = document.createElement('a');
                a.href = url;
                a.download = `bicep-client-${Date.now()}.csv`;
                a.click();
                URL.revokeObjectURL(url);
              }}>
